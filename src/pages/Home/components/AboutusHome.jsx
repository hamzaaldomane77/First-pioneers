      ...
      <div className="w-full flex flex-col-reverse md:flex-row items-center justify-center px-11">
        ...
        <div className={`w-full md:w-1/2 px-11 transition-all duration-1000 ${inView ? 'translate-x-0 opacity-100' : '-translate-x-40 opacity-0'} ${loading ? 'animate-pulse' : ''}`}> 
          {error ? (
            ...
            <div 
              className={`text-lg leading-8 text-black sm:text-sm md:text-base whitespace-pre-line ${isRTL ? 'text-right' : 'text-left'}`}
          ...
          <Link to='/AboutUs' className={`text-xl font-bold cursor-pointer text-[#BB2632] pt-6 sm:text-base md:text-lg ${isRTL ? 'text-right' : 'text-left'}`}>
